"use client";

import { useState, useCallback } from "react";
import BarcodeDisplay from "@/components/barcode/BarcodeDisplay";
import StatusBanner from "@/components/StatusBanner";
import CopyButton from "@/components/CopyButton";

interface Result { svgString: string; pngDataUrl: string; fullCode: string }

const inputClass = "w-full rounded-lg border px-3 py-2 text-sm font-mono outline-none focus:ring-2 focus:ring-amber-400/50 transition";
const inputStyle = { borderColor: "var(--border-strong)", backgroundColor: "var(--bg-elevated)", color: "var(--text-primary)" };

export default function UPCGeneratorClient() {
  const [raw, setRaw] = useState("");
  const [autoCheck, setAutoCheck] = useState(true);
  const [result, setResult] = useState<Result | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);

  const generate = useCallback(async () => {
    const v = raw.trim().replace(/\D/g, "");
    setError(null);
    setResult(null);

    let full = "";
    if (autoCheck) {
      if (v.length !== 11) { setError("Please enter exactly 11 digits. The 12th check digit will be calculated automatically."); return; }
      try {
        const { calcUPCACheckDigit } = await import("@/lib/barcode-generate");
        full = v + calcUPCACheckDigit(v);
      } catch (err) {
        setError(err instanceof Error ? err.message : "Invalid UPC-A value."); return;
      }
    } else {
      if (v.length !== 12) { setError("Please enter exactly 12 digits for a complete UPC-A code."); return; }
      const { validateUPCA } = await import("@/lib/barcode-generate");
      if (!validateUPCA(v)) { setError(`Check digit is incorrect for "${v}". The last digit should be ${await (async () => { const { calcUPCACheckDigit } = await import("@/lib/barcode-generate"); return calcUPCACheckDigit(v.slice(0,11)); })()}.`); return; }
      full = v;
    }

    setLoading(true);
    try {
      const { generateBarcode } = await import("@/lib/barcode-generate");
      const res = await generateBarcode(full, { format: "UPC", width: 2, height: 100, displayValue: true, margin: 10 });
      setResult({ ...res, fullCode: full });
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to generate UPC barcode.");
    } finally {
      setLoading(false);
    }
  }, [raw, autoCheck]);

  const reset = () => { setResult(null); setError(null); setRaw(""); };

  // Format display: X XXXXX XXXXX X
  const formatted = (code: string) =>
    code.length === 12 ? `${code[0]} ${code.slice(1, 6)} ${code.slice(6, 11)} ${code[11]}` : code;

  return (
    <div className="space-y-6">
      <div className="rounded-xl border p-5 space-y-4" style={{ borderColor: "var(--border)", backgroundColor: "var(--bg-surface)" }}>
        <div>
          <label htmlFor="upc-input" className="block mb-1 text-xs font-medium" style={{ color: "var(--text-muted)" }}>
            {autoCheck ? "UPC-A digits (11 digits)" : "Complete UPC-A (12 digits)"}
          </label>
          <input id="upc-input" type="text" inputMode="numeric" value={raw} maxLength={autoCheck ? 11 : 12}
            placeholder={autoCheck ? "03600029145" : "036000291452"}
            onChange={(e) => setRaw(e.target.value.replace(/\D/g, "").slice(0, autoCheck ? 11 : 12))}
            onKeyDown={(e) => e.key === "Enter" && generate()}
            className={inputClass} style={inputStyle} />
          <p className="mt-1 text-[11px]" style={{ color: "var(--text-subtle)" }}>
            {autoCheck
              ? "Enter 11 digits — the 12th check digit is calculated automatically."
              : "Enter all 12 digits including the check digit."}
          </p>
        </div>

        <div className="flex items-center gap-3">
          <input id="upc-auto" type="checkbox" checked={autoCheck}
            onChange={(e) => { setAutoCheck(e.target.checked); setError(null); setResult(null); }}
            className="h-4 w-4 cursor-pointer rounded accent-amber-400" />
          <label htmlFor="upc-auto" className="cursor-pointer text-sm" style={{ color: "var(--text-secondary)" }}>
            Auto-calculate check digit
          </label>
        </div>
      </div>

      <div className="flex flex-wrap gap-3">
        <button type="button" onClick={generate} disabled={loading}
          className="focus-ring inline-flex items-center gap-2 rounded-lg px-6 py-2.5 text-sm font-semibold transition hover:opacity-90 disabled:opacity-50"
          style={{ backgroundColor: "var(--accent)", color: "var(--accent-fg)" }}>
          {loading ? "Generating…" : "Generate UPC Barcode"}
        </button>
        {result && (
          <button type="button" onClick={reset}
            className="focus-ring inline-flex items-center rounded-lg border px-5 py-2.5 text-sm font-medium transition"
            style={{ borderColor: "var(--border-strong)", backgroundColor: "var(--bg-elevated)", color: "var(--text-secondary)" }}>
            Reset
          </button>
        )}
      </div>

      {error && <StatusBanner type="error" message={error} />}

      {result && (
        <div className="space-y-4">
          <BarcodeDisplay svgString={result.svgString} pngDataUrl={result.pngDataUrl} value={result.fullCode} />
          <div className="rounded-lg border p-4 flex items-center justify-between gap-4"
            style={{ borderColor: "var(--border)", backgroundColor: "var(--bg-surface)" }}>
            <div>
              <p className="text-[11px] uppercase tracking-wide font-semibold mb-1" style={{ color: "var(--text-subtle)" }}>Generated UPC-A</p>
              <p className="font-mono text-lg font-bold" style={{ color: "var(--text-primary)" }}>{formatted(result.fullCode)}</p>
            </div>
            <CopyButton text={result.fullCode} />
          </div>
        </div>
      )}

      <p className="text-[11px]" style={{ color: "var(--text-subtle)" }}>
        🔒 All processing happens locally in your browser. No data is uploaded.
      </p>
    </div>
  );
}
