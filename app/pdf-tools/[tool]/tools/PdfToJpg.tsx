...
export default function PdfToJpg() {
  ...
  const handleFiles = useCallback(async (files: File[]) => {
    ...
    const result = await pdfToImages(bytes, "jpeg", 2);
  ...
  return (
    <PdfToolShell
      status={state.status}
      processingLabel="Rendering pages to JPG…"
      error={state.error}
      ...
      actions={
        state.pages.length > 0 ? (
          <>
            {state.pages.map((p, i) => (
              <DownloadBtn key={i} href={state.urls[i]} filename={`${base}-page-${p.page}.jpg`} label={`Page ${p.page} (${formatBytes(p.blob.size)})`} />
            ))}
          </>
        ) : null
      }
