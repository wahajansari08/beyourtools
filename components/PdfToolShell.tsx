...
interface Props {
  ...
  warning?: string | null;
  info?: string | null;
  /** Shown when status is idle */
  dropzone: ReactNode;
  /** Shown when status is done */
  result?: ReactNode;
  /** Extra option controls shown above the dropzone */
  toolbar?: ReactNode;
    ...
    <div className="space-y-4">
      ...
      {status === "processing" && (
        <div className="flex flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed border-ink-600 bg-ink-900 p-14 text-center">
          ...
          <p className="text-sm text-mist-300">{processingLabel}</p>
        </div>
      )}
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-ink-700 bg-ink-900">
      <div className="flex items-center gap-2 border-b border-ink-700 px-3 py-2">
        <span className={clsx("h-1.5 w-1.5 rounded-full", status === "error" ? "bg-coral-400" : "bg-teal-400")} />
